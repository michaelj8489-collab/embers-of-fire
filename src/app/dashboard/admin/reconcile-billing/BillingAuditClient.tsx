...
type ReconcileResult = {
  ...
  activeProfilesWithoutStripeMatch?: Array<{
    ...
    tier: string | null;
    status: string | null;
    accessKind: 'manual_admin_or_staff' | 'unmatched_paid_profile';
  }>;
  issues?: Array<Record<string, unknown>>;
  note?: string;
};

export default function BillingAuditClient() {
  ...
  const run = async (mode: 'dry-run' | 'apply') => {
    if (mode === 'apply') {
      const confirmed = window.confirm(
        'Backfill only the unambiguous matches into Supabase? This does NOT update Stripe, create invoices, change billing dates, or trigger charges.'
      );
      if (!confirmed) return;
    }
    ...
    <div className="min-h-screen bg-black text-gray-100">
      ...
      <main className="mx-auto max-w-6xl px-4 pb-20 pt-32">
        <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="font-cinzel text-sm uppercase tracking-[0.25em] text-orange-400">Owner-only audit • MFA verified</p>
        ...
        <section className="mb-8 rounded-2xl border border-orange-900/60 bg-zinc-950 p-6">
          <p className="leading-relaxed text-gray-300">
            Dry Run reads live Stripe subscription/customer/product data and compares it with Supabase without changing either system.
            Apply writes only verified membership mappings into Supabase; it does not alter Stripe subscriptions, invoices, billing dates, or payments.
          </p>
          <div className="mt-4 rounded-xl border border-emerald-900/60 bg-emerald-950/20 p-4 text-sm text-emerald-100">
            MONEY-SAFE MODE: neither audit action creates a Stripe charge. Plan changes remain a separate feature and are not called from this page.
          </div>
          ...
          <section className="space-y-6">
            {!result.success ? (
              ...
              <>
                ...
                <ResultBlock title="Manual / Non-billing Access Profiles" count={(result.activeProfilesWithoutStripeMatch ?? []).filter((item) => item.accessKind === 'manual_admin_or_staff').length}>
                  {(result.activeProfilesWithoutStripeMatch ?? [])
                    .filter((item) => item.accessKind === 'manual_admin_or_staff')
                    .map((item) => (
                      <div key={item.userId} className="rounded-lg border border-sky-800/50 bg-sky-950/20 p-4 text-sm">
                        <div className="font-semibold text-sky-200">{item.email ?? item.userId}</div>
                        <div className="text-gray-300">{item.role ?? 'admin/staff'} • {item.tier} • {item.status} • no Stripe match expected</div>
                      </div>
                    ))}
                </ResultBlock>

                <ResultBlock title="Non-admin Profiles Without a Stripe Match" count={(result.activeProfilesWithoutStripeMatch ?? []).filter((item) => item.accessKind === 'unmatched_paid_profile').length}>
                  {(result.activeProfilesWithoutStripeMatch ?? [])
                    .filter((item) => item.accessKind === 'unmatched_paid_profile')
                    .map((item) => (
                ...
                <ResultBlock title="Stripe Exceptions Left Unchanged" count={result.issues?.length ?? 0}>
